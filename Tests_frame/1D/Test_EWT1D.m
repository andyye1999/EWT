...
fileId = fopen('E:\Matlab_workspace\Tests_frame\1D\¹Çµ¼²Î¿¼ÂË²¨sin2468.pcm','r');
sin100500 = fread(fileId,inf,'int16');
% sin100500 = audioread('E:\Matlab_workspace\Tests\1D\cs375_20000.wav');
fid=fopen('E:\Matlab_workspace\Tests_frame\Resignal.pcm','wb');
for frame_num = 0:9
%     kkk = 0;
    cs_size = 1000;
    f = sin100500((frame_num*cs_size+1):(frame_num*cs_size+cs_size));

    % xxx = 1:1024;
    % f = 5*sin(1000*2*pi/16000*xxx) + 2.5*sin(3500*2*pi/16000*xxx)+wgn(1,1024,1);
    % t=0:length(f)-1;

    % xxx = 1:1024;
    % f = sin(500*2*pi/16000*xxx) + sin(1500*2*pi/16000*xxx);
    % t=0:length(f)-1;
    ...
    if Rec==1
            ...
            for ewi = 1:length(ewt)
                if ewt_max_num(ewi) > ewt_max_num(1)*0.3
                    ewt_index = ewi;
                end
            end
            for i = 1:ewt_index
...
%             for i = 1:FADE_Num
%                 rec(i) = (i - 1)/FADE_Num*rec(i);
%                 rec(length(rec) - i + 1) = (i - 1)/FADE_Num*rec(length(rec) - i + 1);
%             end
            figure;
